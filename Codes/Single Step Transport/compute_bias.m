function l2_bias = compute_bias(para, samples, beta, weights)
    ...
    %
    % Inputs:
    %   para    : Potential object (e.g., Three_Well). Must contain:
    %             - Properties: X_LIM_COMPUTE, Y_LIM_COMPUTE
    ...
    x_lim = para.X_LIM_COMPUTE;
    y_lim = para.Y_LIM_COMPUTE;
    ...
    % These functions capture different statistical moments and modes
    funcs = {
        @(x) sin(x(:,1)),  'sin(x1)';
        @(x) cos(x(:,1)),  'cos(x1)';
        @(x) sin(x(:,2)),  'sin(x2)';
