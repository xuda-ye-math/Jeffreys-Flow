...
class Normalizing_Flow(nn.Module):
    ...
    def __init__(self):
        ...
        # Note: GMM 3D uses X/Y/Z_LIM_COMPUTE, all are [-12, 12]
        # We simplify and use X_LIM as the global limit for all dimensions
        ...
        print(f"        Limits: {x_limits} (Isotropic)")

        for i in range(num_layers):
            mask_parity = i % 2
            # For 3D GMM, we assume isotropic boundaries, so we pass the same limits
...
if __name__ == "__main__":
    ...
    # Determine bounds for random check
    x_min = para.X_LIM_COMPUTE[0]
    x_max = para.X_LIM_COMPUTE[1]

    # Check if Z limits exist (for 3D robustness)
    if hasattr(para, 'Z_LIM_COMPUTE'):
        z_min = para.Z_LIM_COMPUTE[0]
        z_max = para.Z_LIM_COMPUTE[1]
        limit_min = max(x_min, z_min)  # Pick inner bound
        limit_max = min(x_max, z_max)
    else:
        limit_min = x_min
        limit_max = x_max
