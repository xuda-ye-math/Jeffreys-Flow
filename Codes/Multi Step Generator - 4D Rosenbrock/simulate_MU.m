...
para = Rosenbrock_4D();   % Initialize Parameter Object (Updated for Linear Interp)
...
if all_files_exist
    ...
    % [UPDATED] Handle Vector MEAN/SIGMA for Dimensions
    % Rosenbrock_4D defines SIGMA as standard deviation vector.
    % mvnrnd expects Covariance Matrix, so we use diag(SIGMA.^2).
    mu_base = para.MEAN; 
    sigma_base = diag(para.SIGMA.^2); 
    ...
    parfor k = 1:N_CORES
        ...
        % [UPDATED] Initialize PT Ladder Locally (Vector Compatible)
        l_mu = local_para.MEAN;
        l_sig = diag(local_para.SIGMA.^2);
...
function [x, acc_vec, att_vec] = perform_swap(x, para, is_even_phase)
    ...
    % Note: para.MEAN and para.SIGMA are vectors, x is [M, D].
    % MATLAB handles element-wise ops (./ and -) correctly here.
