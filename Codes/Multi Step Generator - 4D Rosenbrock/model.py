...
class Coupling_Layer(nn.Module):
    ...
    Adapted for general N-dimensional input logic.
    """
    ...
    def __init__(self, dim, hidden_dim, num_bins, mask_parity, limits):
        ...
        # Parity 0: ID = First Part (split_dim), TR = Second Part (dim - split_dim)
        # Parity 1: TR = First Part (split_dim), ID = Second Part (dim - split_dim)
        if mask_parity == 0:
        ...
        # Context Network
        # Input: self.id_dim -> Output: self.tr_dim * param_dim
...
class Normalizing_Flow(nn.Module):
    ...
    This implementation automatically calculates the required bounding box
    for spline normalization based on the parameter configuration.
    """
    ...
    def __init__(self):
        ...
        # ---- Calculate Robust Global Limits (Union of all Dimensions) ----
        # Scan 'para' for X1_LIM_COMPUTE, X2_LIM_COMPUTE, etc.
        # and create a bounding box that covers the union of all anisotropic dimensions.
        mins = []
        maxs = []

        # Dimension-Specific Limits (X1..X4_LIM_COMPUTE)
        # We STRICTLY check for X{d}_LIM_COMPUTE only.
        # Removed generic X_LIM_COMPUTE fallback as requested.
        for d in range(1, dim + 1):
            attr = f"X{d}_LIM_COMPUTE"
            if hasattr(para, attr):
                lim = getattr(para, attr)
                mins.append(float(lim[0]))
                maxs.append(float(lim[1]))
            else:
                # Warning if a specific dimension limit isn't defined
                print(f"[Model] Warning: {attr} not found. Using default [-10, 10] for this dimension.")
                mins.append(-10.0)
                maxs.append(10.0)

        # Construct Box (Union of all limits)
        if len(mins) > 0:
            global_min = min(mins)
            global_max = max(maxs)
            x_limits = [global_min, global_max]
        else:
            print("[Model] Warning: No limits found in 'para'. Using default [-10, 10].")
            x_limits = [-10.0, 10.0]

        print(f"[Model] Initializing {num_layers}-layer Spline Flow.")
        print(f"        Dimensions: {dim}")
        print(f"        Global Spline Limits: {x_limits} (computed strictly from X1..X{dim}_LIM_COMPUTE)")
