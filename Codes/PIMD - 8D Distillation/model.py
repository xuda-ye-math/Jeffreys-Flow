...
# ---- 1. Rational Quadratic Spline Implementation ----
...
class Coupling_Layer(nn.Module):
    ...
    def __init__(self, dim, hidden_dim, num_bins, mask_parity, limits):
        ...
        Args:
            ...
            limits (tensor): Shape [dim, 2] containing [min, max] per dimension.
        """
        super().__init__()
        self.dim = dim
        ...
        if mask_parity == 0:
            ...
            self.tr_indices = list(range(self.split_dim, dim))
        else:
            self.id_dim = dim - self.split_dim
            self.tr_dim = self.split_dim
            self.tr_indices = list(range(0, self.split_dim))

        # Set bounds (per transformed dimension)
        # We store these as buffers so device transfers happen seamlessly
        self.register_buffer('limit_min', limits[self.tr_indices, 0])
        self.register_buffer('limit_max', limits[self.tr_indices, 1])
    ...
    def _rational_quadratic_spline(self, inputs, unnormalized_widths, unnormalized_heights, unnormalized_derivatives,
        ...
        # inputs is heavily flattened to [N]. We need to broadcast limit_min and limit_max.
        num_elements = inputs.shape[0]
        num_tiles = (num_elements + self.tr_dim - 1) // self.tr_dim
        left = self.limit_min.repeat(num_tiles)[:num_elements]
        right = self.limit_max.repeat(num_tiles)[:num_elements]
        ...
        left_in = left[inside_interval_mask]
        right_in = right[inside_interval_mask]

        # 3. Spline Transformation
        if inverse:
            # --- Inverse (Generative: z -> x) ---
            inputs_norm = (inputs_in - left_in) / (right_in - left_in)
            ...
            outputs[inside_interval_mask] = outputs_norm * (right_in - left_in) + left_in
...
class Normalizing_Flow(nn.Module):
    ...
    def __init__(self, pt):
        """
        Initializes the normalizing flow using an instance of Path_Integral_Potential.
        """
        super().__init__()
        ...
        # Calculate limits structurally from pt parameters
        SIGMA = pt.SIGMA
        limits = torch.zeros(dim, 2)
        
        # Mode 0 bounds: [-3 * sqrt(BETA) * SIGMA, 3 * sqrt(BETA) * SIGMA]
        limit_0 = 3.0 * math.sqrt(pt.beta) * SIGMA
        limits[0, 0] = -limit_0
        limits[0, 1] = limit_0
        
        # Mode k bounds: [-3 / omega_k, 3 / omega_k]
        omega = pt.OMEGA
        for k in range(1, dim):
            limit_k = 3.0 / omega[k].item()
            limits[k, 0] = -limit_k
            limits[k, 1] = limit_k

        print(f"[Model] Initializing {num_layers}-layer Spline Flow.")
        print(f"        Dimensions: {dim}")
        print(f"        Global Spline Limits: \n{limits}")

        for i in range(num_layers):
            mask_parity = i % 2
            self.layers.append(Coupling_Layer(dim, hidden_dim, num_bins, mask_parity, limits=limits))
...
if __name__ == "__main__":
    ...
    pt_test = Path_Integral_Potential(N=8, beta=1.0)
    print(f"--- Verifying Model (Dim={pt_test.N}) ---")

    model = Normalizing_Flow(pt_test)
    model.eval()

    # 1. Identity Check
    B = 10

    # Retrieve limits generically per dimension to build a fully contained test distribution
    limit_min = model.layers[0].limit_min.unsqueeze(0)
    limit_max = model.layers[0].limit_max.unsqueeze(0)
    limit_min_id = model.layers[1].limit_min.unsqueeze(0)
    limit_max_id = model.layers[1].limit_max.unsqueeze(0)

    # Reassemble total min/max arrays simulating uniform sampling strictly inside the regions
    limits_all_min = torch.zeros(1, pt_test.N)
    limits_all_max = torch.zeros(1, pt_test.N)
    
    limits_all_min[:, model.layers[0].tr_indices] = limit_min
    limits_all_max[:, model.layers[0].tr_indices] = limit_max
    limits_all_min[:, model.layers[1].tr_indices] = limit_min_id
    limits_all_max[:, model.layers[1].tr_indices] = limit_max_id

    # Ensure points are strictly inside to trigger spline logic per dimension
    width = limits_all_max - limits_all_min
    x = torch.rand(B, pt_test.N) * (width - 1e-1) + (limits_all_min + 0.5e-1)
