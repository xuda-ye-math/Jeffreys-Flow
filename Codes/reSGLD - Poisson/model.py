...
class Coupling_Layer(nn.Module):
    ...
    def _rational_quadratic_spline(self, inputs, unnormalized_widths, unnormalized_heights, unnormalized_derivatives,
        ...
        # For Poisson inversion, variables are strict coordinates in [0, 1].
        # We apply the spline ONLY inside the bounding box.
...
class WeightNet(nn.Module):
    ...
    Used for computing incremental importance weights accurately.
    ...
    def __init__(self, dim):
        ...
        self.num_layers = max(4, NUM_LAYERS // 2) # Slightly shallower for weight estimation
...
class Normalizing_Flow(nn.Module):
    ...
    STRICTLY relies on 'X_LIM_COMPUTE' for spline limits [0, 1].
    """
...
if __name__ == "__main__":
    ...
    # 1. Identity Check within [0, 1] Bounds
    B = 1000
    ...
    # x ~ Uniform(0.01, 0.99)
    x = torch.rand((B, para.DIM), device=device) * (width - 0.02) + (limit_min + 0.01)
    ...
    print(f"\nProcessed {B} items through full cycle in {t1-t0:.4f} seconds.")

    # Final Verdict
    if diff_id_fwd < 1e-4 and diff_id_inv < 1e-4 and max_ld_fwd < 1e-4 and max_ld_inv < 1e-4:
        print("\n>> SUCCESS: Model initialized as Strict Identity within [0, 1].")
    else:
