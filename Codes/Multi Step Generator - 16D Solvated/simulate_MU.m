...
para = Solvated_16D();   % Initialize Parameter Object (Twisted Nonlinear Rastrigin)
...
if all_files_exist
    ...
    % Generate samples:
    % Dims 1-2: Uniform(X_PERI_LIM)
    % Dims 3-16: Gaussian(MEAN, SIGMA)
    
    samples_MU_0 = zeros(para.MU_SIZE, para.DIM);
    
    % 1. Periodic Dims (Uniform)
    u_min = para.X_PERI_LIM(1);
    u_max = para.X_PERI_LIM(2);
    samples_MU_0(:, 1:2) = u_min + (u_max - u_min) .* rand(para.MU_SIZE, 2);
    
    % 2. Bath Dims (Gaussian)
    mu_bath = zeros(1, para.DIM-2) + para.MEAN; 
    sigma_bath = eye(para.DIM-2) * (para.SIGMA^2);
    samples_MU_0(:, 3:end) = mvnrnd(mu_bath, sigma_bath, para.MU_SIZE);
    ...
    parfor k = 1:N_CORES
        ...
        % Initialize Replicas Locally (Start from Mixed Topology)
        x = zeros(N_REPLICAS, local_para.DIM);
        
        % Dims 1-2: Uniform(X_PERI_LIM) 
        % This is crucial for the Grid Topology on T^2
        x(:, 1:2) = u_min + (u_max - u_min) .* rand(N_REPLICAS, 2);
        
        % Dims 3-16: Gaussian (Bath)
        x(:, 3:end) = mvnrnd(mu_bath, sigma_bath, N_REPLICAS);
        ...
        for i = 1:local_samples_per_core
            ...
            if k == 1
                if mod(i, 500) == 0 || i == local_samples_per_core
