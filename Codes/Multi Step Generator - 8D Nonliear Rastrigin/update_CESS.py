...
SCRIPT_DIR = '/media/xuda/Data/Rare Event Sampling/Codes/Multi Step Generator - 8D Nonliear Rastrigin'
...
def main():
    ...
    import csv
    csv_path = os.path.join(DATA_DIR, f'{PREFIX}_CESS.csv')
    csv_file = open(csv_path, 'w', newline='')
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['Step', 'ESS', 'CESS'])
    ...
    for k in range(0, M+1):
        if k == 0:
            ...
            csv_writer.writerow([0, f"{ess_0.item():.2%}", "N/A"])
            continue
        ...
        csv_writer.writerow([k, f"{ess_k.item():.2%}", f"{cess_k:.2%}"])
