...
class Coupling_Layer(nn.Module):
    ...
    def __init__(self, dim, hidden_dim, num_bins, mask_parity, limits):
        ...
        # Structure per dim: [Widths (K) | Heights (K) | Derivatives (K+1)]
        self.num_bins = num_bins
        self.param_dim = 3 * num_bins + 1

        # Context Network
        # Input: split_dim -> Output: split_dim * param_dim
        ...
        with torch.no_grad():
            ...
            # softplus(val) + min_d approx 1.0
            min_derivative = 1e-3
            target_val = 1.0 - min_derivative
            # Inverse Softplus
            inv_softplus_val = np.log(np.exp(target_val) - 1)

            # Reshape bias to [out_dims, param_dim] to access derivative slots
            ...
            # Set them to inv_softplus_val so that softplus(d) = 1.0
    ...
    def _rational_quadratic_spline(self, inputs, unnormalized_widths, unnormalized_heights, unnormalized_derivatives,
        ...
        # widths: Softmax to sum to 1, scaled to range
        ...
        # derivatives: Softplus to be positive
        derivatives = F.softplus(unnormalized_derivatives) + min_derivative
        derivatives = F.pad(derivatives, (1, 1), mode='constant', value=1.0)  # Pad ends with 1 (identity at boundaries)
        ...
        if inverse:
            # --- Inverse (Generative Direction: z -> x) ---
            ...
            # Chain rule adjustment for bounds
    ...
    def forward(self, x, inverse=False):
        ...
        """
        # Split features
        # parity 0: ID=First Half, TR=Second Half
        # parity 1: TR=First Half, ID=Second Half
...
class WeightNet(nn.Module):
    """
    Network for phi and psi functions (R^d -> R).
    Uses parameters from parameters.py.
    """

    def __init__(self, dim):
        super().__init__()
        # Use parameters from global scope (imported from parameters.py)
        ...
        # Constant Initialization (Zero for last layer to ensure f(x) start approx 0)
        with torch.no_grad():
            self.net[-1].weight.fill_(0.0)
            self.net[-1].bias.fill_(0.0)
...
class Normalizing_Flow(nn.Module):
    ...
    def __init__(self):
        ...
        # Read Bounds from para object
        x_limits = [float(para.X_LIM_COMPUTE[0]), float(para.X_LIM_COMPUTE[1])]
        y_limits = [float(para.Y_LIM_COMPUTE[0]), float(para.Y_LIM_COMPUTE[1])]
        ...
        for i in range(num_layers):
            ...
            # Assign limits based on which dimension is being TRANSFORMED
            if mask_parity == 0:
                layer_limits = y_limits
            else:
                layer_limits = x_limits

            self.layers.append(Coupling_Layer(dim, hidden_dim, num_bins, mask_parity, limits=layer_limits))
...
if __name__ == "__main__":
    print(f"--- Verifying Model for {para.NAME} ---")
    ...
    # 1. Identity Check
    B = 10
    limit = min(para.X_LIM_COMPUTE[1], para.Y_LIM_COMPUTE[1])
    # Ensure points are strictly inside to trigger spline logic
    x = (torch.rand(B, para.DIM) * 2 - 1) * (limit - 0.5)
