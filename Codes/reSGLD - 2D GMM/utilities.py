...
def _mala_core(para, samples, lam, dt, n_iterations):
    """ Internal MALA loop for a specific batch (SVRG Variance Reduction) """
    x = samples.clone()
    N = x.shape[0]
    device = x.device
    sqrt_2dt = np.sqrt(2 * dt)

    # 1. Compute Full Gradient at Reference Point (Initial Batch)
    # x_ref is fixed for the duration of this call
    x_ref = x.clone()
    with torch.no_grad():
        full_grad_ref = para.compute_gradient_mixed(x_ref, lam, i_vec=None)

    for _ in range(n_iterations):
        # Generate i_vec for Stochastic Potential (if applicable)
        # We assume para has NOISE attribute, or default to 0
        noise_level = getattr(para, 'NOISE', 4.0)
        signs = torch.randint(0, 2, (N, 2), device=device).float() * 2 - 1
        i_vec = signs * noise_level

        with torch.no_grad():
            # 2. Compute Stochastic Gradient with Variance Reduction
            # grad_approx = grad(x, i_vec) - grad(x_ref, i_vec) + full_grad_ref
            
            grad_x_stoch = para.compute_gradient_mixed(x, lam, i_vec=i_vec)
            grad_ref_stoch = para.compute_gradient_mixed(x_ref, lam, i_vec=i_vec)
            
            grad_x = grad_x_stoch - grad_ref_stoch + full_grad_ref

            # 3. Proposal (Langevin Step)
            noise = torch.randn_like(x)
            # ULA Update (Always Accept)
            x = x - dt * grad_x + sqrt_2dt * noise
