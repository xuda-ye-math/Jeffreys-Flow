...
EPOCHS_F = 40 # epochs for flow training
EPOCHS_W = 40 # epochs for weight training
BATCH_SIZE = 4000
LR_F = 1e-4
LR_W = 1e-4
WEIGHT_DECAY = 0.0
...
def load_data(device='cpu', mode='correct'):
    """
    ...
    """
    print(f"[Data] Scanning and loading all MATLAB data from {DATA_DIR} (Mode: {mode})...")

    # 1. Load Samples
    # Pattern to match {ABBR}_samples_{mode}_MU_{index}.mat
    pattern = rf"^{ABBR}_samples_{mode}_MU_(.+)\.mat$"
    data_dict = {}
...
from potential.GMM_2D import GMM_2D
